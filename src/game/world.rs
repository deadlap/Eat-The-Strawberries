...
use ggez::{
    Context, GameResult,
    error::GameError,
    graphics::{spritebatch::{SpriteBatch, SpriteIdx}},
};
        ...
        loop {
            ...
            match &*buf.trim_end() {
                ...
                "PALETTE" => ret.palette = bincode::deserialize_from(&mut reader)
                    .map(|names: Vec<&str>| Palette::new(names))
...
impl Grid {
    ...
    pub fn draw(&self, pal: &Palette, ctx: &mut Context, textures: &Textures) -> GameResult<()> {
        for (i, &mat) in self.mats.iter().enumerate() {
            let x = f32::from(i as u16 % self.width) * 32.;
            let y = f32::from(i as u16 / self.width) * 32.;

            pal.draw_mat(mat, ctx, textures, x, y, Default::default())?;
        }
        Ok(())
    }
}
